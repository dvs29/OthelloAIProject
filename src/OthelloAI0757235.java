...
public class OthelloAI0757235 implements OthelloAI {
    ...
    public OthelloMove chooseMove(OthelloGameState state) {
        ...
        // initialize MCTS at the start of the game.
        if(mcts == null) {
    ...
    public class Mcts {
        ...
        private Node getGameStateAsRootNode(OthelloGameState state) {
            ...
            if(hash == ZobristHashing.getHash(root.getGameState())) {
                return root;
            }
        ...
        private void backpropogation(Node simulatedNode, OthelloGameState finalState) {
            ...
            while (tempNode != null) {
                tempNode.addVisitCount(1); //increment visit count for the node
     ...
     * More information here - https://en.wikipedia.org/wiki/Zobrist_hashing
     */
    public static final class ZobristHashing {
        private static final int[][] BIT_STRINGS = new int[3][64];

        // Randomly generates bit strings for each possible element of a board game (3 X 64)
        static {
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 64; j++) {
